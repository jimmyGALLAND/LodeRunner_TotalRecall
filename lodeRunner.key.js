...
function handleKeyDown(event) {
    ...
    /*} else if (event.ctrlKey) {
        ...
        if ((gameState == GAME_PAUSE && code === "Escape") ||
    ...
    // Autoriser les touches F1 à F12
    if (code.startsWith("F")) return true;
    return false;
}
...
function handleKeyUp(event) {
    ...
    if (!event) event = window.event;
    const code = event.code;
    if (recordKeyCode === code && keyPressed != -1) keyPressed = 0;
    return true;
}
